...
import 'package:bugfix/screens/createTicketScreen.dart';
...
// Add this helper method to determine card type based on BIN
  ...
  Widget _buildTransactionCarousel(Map<String, dynamic> responseData) {
    ...
    final terminalTransactionsRaw = (responseData['lastFiveTxnAmount'] as List?) ?? [];
    final qrTransactionsRaw = (responseData['lastFiveTxnAmountForQr'] as List?) ?? [];

    List<Map<String, dynamic>> allTransactions = [];

    // Helper to parse POS timestamp
    DateTime? parsePosTimestamp(String? timestampStr) {
      if (timestampStr == null || timestampStr.isEmpty) return null;
      try {
        // Input format: "24 Jun 2025 07.08 PM"
        return DateFormat('dd MMM yyyy hh.mm a', 'en_US').parseLoose(timestampStr).toLocal();
      } catch (e) {
        print('Error parsing POS timestamp "$timestampStr": $e');
        return null;
      }
    }

    // Helper to parse QR timestamp
    DateTime? parseQrTimestamp(String? timestampStr) {
      if (timestampStr == null || timestampStr.isEmpty) return null;
      try {
        // Input format: "2025-05-06T11:31:17+05:30"
        return DateTime.parse(timestampStr).toLocal();
      } catch (e) {
        print('Error parsing QR timestamp "$timestampStr": $e');
        return null;
      }
    }

    // Process terminal transactions
    for (var txData in terminalTransactionsRaw) {
      final tx = txData as Map<String, dynamic>;
      final status = _getTransactionStatus(tx['txnType'], tx['txnResponseCode'] ?? 'Unknown');
      final timestampStr = tx['responseReceivedTime']?.toString();
      final dateTime = parsePosTimestamp(timestampStr);

      allTransactions.add({
        "id": tx['terminalId']?.toString() ?? '',
        "amount": formatCurrency((tx['txnAmount'] as num? ?? 0) + (tx['txnAdditionalAmount'] as num? ?? 0)),
        "status": status,
        "time": _formatCarouselDisplayDateTime(timestampStr, true), // For display
        "unifiedTimestamp": dateTime ?? DateTime(1970), // For sorting, default to old date if parsing fails
        "logo": _getCardLogo(_getCardType(tx)),
        "type": _getCardType(tx), // POS or card type
        "rrn": tx['rRNumber']?.toString() ?? '',
        "rawTxnType": tx['txnType']?.toString() ?? '',
        "rawResponseCode": tx['txnResponseCode']?.toString() ?? '',
        "originalDataSource": tx, // Keep original data
        "isPosTransaction": true // Flag to help with display formatting later
      });
    }

    // Process QR transactions
    for (var txData in qrTransactionsRaw) {
      final tx = txData as Map<String, dynamic>;
      final status = _getQRTransactionStatus(tx['gatewayResponseCode'] ?? 'Unknown');
      final timestampStr = tx['transactionTimestamp']?.toString();
      final dateTime = parseQrTimestamp(timestampStr);

      allTransactions.add({
        "id": tx['customerVpa']?.toString() ?? '',
        "amount": formatCurrency(double.parse(tx['transactionAmount']?.toString() ?? '0')),
        "status": status,
        "time": _formatCarouselDisplayDateTime(timestampStr, false), // For display
        "unifiedTimestamp": dateTime ?? DateTime(1970), // For sorting, default to old date if parsing fails
        "originalFullTimestamp": timestampStr ?? '', // Keep for display formatting if needed
        "logo": "assets/qr.png",
        "type": "QR", // Explicitly QR type
        "rrn": tx['rRNumber']?.toString() ?? '', // Assuming rRNumber exists, adjust if different for QR
        "rawTxnType": tx['purposeCode']?.toString() ?? '',
        "rawResponseCode": tx['gatewayResponseCode']?.toString() ?? '',
        "originalDataSource": tx, // Keep original data
        "isPosTransaction": false // Flag to help with display formatting later
      });
    }

    // Sort all transactions by unifiedTimestamp in descending order (most recent first)
    allTransactions.sort((a, b) {
      DateTime dtA = a['unifiedTimestamp'] as DateTime;
      DateTime dtB = b['unifiedTimestamp'] as DateTime;
      return dtB.compareTo(dtA); // Descending order
    });

    // The limiting to top 10 will be done in the next step.
    // For now, combinedTransactions will be the fully sorted list.
    // List<Map<String, dynamic>> combinedTransactions = allTransactions; // Placeholder for now, will be limited next

    // Step 3: Take Top 10 (or fewer if less than 10 available)
    List<Map<String, dynamic>> topTransactions = allTransactions.take(10).toList();
    List<Map<String, dynamic>> combinedTransactions = topTransactions;
